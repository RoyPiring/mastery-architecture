...
**Catalog: `ideas/mastery-architecture-100/mastery-architecture-100.md`**

Do-first order lives in `ideas/mastery-architecture-100/mastery-architecture-100-RANKING.md`. Start at rank 1 and go in order.

| Rank | Start with |
|------|------------|
| 1 | The House-Design Concept Map |
| 2 | The Routes: PhD, Architect License, Building Designer |
| 3 | The House-Design Process, End to End |
| 4 | Reading a House: Precedent Study |
...
The barrier here is unusually low and worth stating plainly: in many US states the SINGLE-FAMILY EXEMPTION lets a building designer or owner design one- and two-family dwellings WITHOUT an architect license. The fitting credential is the AIBD Certified Professional Building Designer via the NCBDC, which is a PRIVATE CREDENTIAL and not a license. Jurisdiction rules vary, and larger, multifamily, or complex work needs a licensed architect and engineers.


## Safety layer (binding)

- **LIFE SAFETY.** The builds here are STUDY and PORTFOLIO MOCK-UPS. They are NOT stamped construction documents, engineering certifications, or code approvals.
- **The title `architect` is legally protected.** Do not use it for yourself unless you are licensed.
- **Life-safety decisions require the licensed professionals responsible for them**, including egress, structure, and fire separation.
- **Build to the IRC prescriptive path** for one- and two-family dwellings; work outside it needs engineered design and local code review.


## The binding contract (what every project here must carry)

- **Point of view.** PoV-M, the learner-principal. Every payload is written as someone building command of designing houses (residential architecture) by doing, never as a textbook narrator. The persona is binding on all payloads (rule 1).
